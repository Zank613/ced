/* ced_v4 - A single-file text editor with syntax highlighting using ncurses.
   Key Features:
   ...
   Compile:  gcc -o ced_v4 main.c -lncurses
   Run:      ./ced_v4
*/
...
{
    if (line >= 0 && line < MAX_LINES) line_dirty[line] = 1;
}
void editor_mark_all_lines_dirty(void)
{
    int i;
    for (i = 0; i < MAX_LINES; i++) line_dirty[i] = 1;
}
...
{
    rule->tokens = NULL; rule->token_count = 0;
...
static SH_SyntaxDefinitions sh_load_syntax_definitions(const char *filename)
{
    ...
    defs.definitions = NULL; defs.count=0;
    {
        ...
        if (!fp) return defs;
        {
            ...
            while (fgets(line, sizeof(line), fp))
            {
                ...
                if (!strncmp(trimmed,"SYNTAX",6))
                {
                    ...
                    def.extensions=NULL; def.ext_count=0;
                    def.rules=NULL; def.rule_count=0;
                    {
                        char *p = trimmed+6;
                        while(*p)
                        {
                    ...
                    while(1)
                    {
                        char rulebuf[4*SH_MAX_LINE_LENGTH]; rulebuf[0]='\0';
                        {
                            int done=0, have_semicolon=0;
                            while(!have_semicolon)
                            {
                                if(!fgets(line,sizeof(line),fp)){done=1;break;}
                                trimmed=trim_whitespace(line);
                                if(trimmed[0]=='}'){done=1;break;}
                                if(!strlen(trimmed)) continue;
                                strncat(rulebuf,trimmed,sizeof(rulebuf)-strlen(rulebuf)-2);
                                strncat(rulebuf," ",sizeof(rulebuf)-strlen(rulebuf)-2);
                                if(strchr(trimmed,';')) {have_semicolon=1;break;}
                            }
                            if(done) break;
                        }
                        {
...
void sh_free_syntax_definitions(SH_SyntaxDefinitions defs)
{
    ...
    for(i=0;i<defs.count;i++)
    {
        ...
        for(j=0;j<d.ext_count;j++) free(d.extensions[j]);
        free(d.extensions);
        for(k=0;k<d.rule_count;k++)
        {
            SH_SyntaxRule rr=d.rules[k];
            for(t=0;t<rr.token_count;t++) free(rr.tokens[t]);
            free(rr.tokens);
        }
        free(d.rules);
    }
...
int sh_file_has_extension(const char *filename, SH_SyntaxDefinition def)
{
    ...
    for(i=0;i<def.ext_count;i++)
    {
        ...
        if(flen>=elen)
        {
            if(!strcmp(filename+flen-elen,ext)) return 1;
        }
    }
    return 0;
}
...
void sh_init_syntax_colors(SH_SyntaxDefinition *def)
{
    ...
    for(i=0;i<def->rule_count;i++)
    {
        ...
        if(can_change_color()) init_color(color_num,r_scaled,g_scaled,b_scaled);
...
void shell_panel_run_command(void)
{
    ...
    while(fgets(shell_output[shell_output_count],MAX_COLS,fp))
    {
        ...
        shell_output_count++;
        if(shell_output_count>=SHELL_PANEL_LINES) break;
    }
    pclose(fp);
}
...
void init_search_color(void)
{
    ...
    {
        short color_num=250;
        if(can_change_color()) init_color(color_num,1000,1000,0);
...
void editor_search(void)
{
    ...
    if(!term[0]) { g_searchActive=0; g_searchTerm[0]='\0'; return; }
    ...
    editor_prompt("New text: ",newstr,sizeof(newstr));
    {
        ...
        for(i=0;i<editor.num_lines;i++)
        {
            ...
            while(1)
            {
                ...
                if(!pos){strncat(out,start,sizeof(buffer)-strlen(out)-1);break;}
                {
...
void editor_refresh_screen(void)
{
...

    {
        ...
        {
            /* Show key bindings */
            mvprintw(status_row,0,"[HELP] Ctrl+Q:Quit  Ctrl+S:Save  Ctrl+O:Open  Ctrl+Z:Undo  Ctrl+Y:Redo  "
                                  "Ctrl+G:Goto  Ctrl+F:Search  Ctrl+R:Replace  Ctrl+W:ShellPanel  Ctrl+E:ShellCmd  "
                                  "Ctrl+H:Hide help");
        }
    }

    if(shell_panel_open) shell_panel_draw();
...
void editor_insert_char(int ch)
{
    ...
    for(i=len;i>=editor.cursor_x;i--) line[i+1]=line[i];
...
void editor_delete_at_cursor(void)
{
    ...
    {
        if(editor.cursor_y==editor.num_lines-1) return;
...
void editor_insert_newline(void)
{
    if(editor.num_lines>=MAX_LINES) return;
    {
        ...
        {
            int indent=0;
            while(line[indent]==' ' && indent<MAX_COLS-1) indent++;
            {
...
void editor_goto_line(void)
{
    ...
    if(!line_str[0]) return;
    {
        int ln=atoi(line_str);
        if(ln<1) ln=1;
        if(ln>editor.num_lines) ln=editor.num_lines;
...
void process_keypress(void)
{
    ...
    if(ch==KEY_MOUSE)
    {
        ...
        if(getmouse(&event)==OK)
        {
            ...
            {
                editor.cursor_y-=3; if(editor.cursor_y<0) editor.cursor_y=0;
                editor_mark_all_lines_dirty();
            }
            else if(event.bstate&BUTTON5_PRESSED)
            {
                editor.cursor_y+=3; if(editor.cursor_y>=editor.num_lines) editor.cursor_y=editor.num_lines-1;
                editor_mark_all_lines_dirty();
            }
        }
        return;
    ...
    switch(ch)
    {
        ...
        case 15: /* Ctrl+O: open */
            ...
            break;
        case KEY_HOME:
            editor.cursor_x=0; editor_mark_line_dirty(editor.cursor_y);
            break;
        case KEY_END:
        {
        ...
        case KEY_PPAGE:
            editor.cursor_y-=5; if(editor.cursor_y<0) editor.cursor_y=0;
            editor_mark_all_lines_dirty();
            break;
        case KEY_NPAGE:
            editor.cursor_y+=5; if(editor.cursor_y>=editor.num_lines) editor.cursor_y=editor.num_lines-1;
            editor_mark_all_lines_dirty();
            break;
        case '\t':
        {
            int i; save_state_undo();
            if(config.tab_four_spaces)
            {
                for(i=0;i<4;i++) editor_insert_char(' ');
            }
            else editor_insert_char('\t');
            break;
        }
        case KEY_LEFT:
            if(editor.cursor_x>0)
...
int main(void)
{
    ...
    initscr(); start_color(); use_default_colors(); raw(); noecho();
    keypad(stdscr,TRUE); curs_set(1); mousemask(ALL_MOUSE_EVENTS,NULL); mouseinterval(0);
    init_editor();
